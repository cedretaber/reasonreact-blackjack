...
let render_card = (number, mark) => {
  ...
  <div className="card">
    <div className="front">
      ...
      <span className={j|bottom-right $colour|j}>(s({j|$number$str|j}))</span>
    </div>
  </div>
};
...
| (Hit, Game.Playing(game_state)) =>  RR.Update(Game.next(Game.Hit, game_state))
| (Stand, Game.Playing(game_state)) => RR.Update(Game.next(Game.Stand, game_state))
  ...
  render: self => {
    ...
    | Game.Win(_) =>
      <div className="result-table">
        <div>
          <span className="result win">(s("You win!"))</span>
        </div>
        <div>
    ...
    | Game.Lose(_) =>
      <div className="result-table">
        <div>
          <span className="result lose">(s("You lose!"))</span>
        </div>
        <div>
          <button onClick=(_ => self.send(Retry))>(s("Retry"))</button>
        </div>
